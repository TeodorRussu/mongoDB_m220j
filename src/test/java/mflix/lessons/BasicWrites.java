...
public class BasicWrites extends AbstractLesson {
    ...
    /**
     * In this lesson we are going to look into writing new data into a collection. Specifically, we are going to review
     * the insert, insertMany and update with {upsert: true} options in the Java driver.
     */
    ...
    /**
     * Let's recap: - we can insert new documents using both the insertOne or insertMany collection methods - update
     * using the $upsert flag set to true also allows us to insert new documents - using $setOnInsert update operator
     * provides a way to set specific fields only in the case of insert.
     */
